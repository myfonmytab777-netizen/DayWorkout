...
function App() {
    ...
    // Map of collapsed days: key format "weekId_dayId" -> boolean
    ...
    // Toggle check status for exercises or time slots
    ...
    // Toggle bukak / tutup (collapse/expand) for a day
                            ...
                            <div key={tpl.id} className="bg-zinc-900/90 border border-zinc-800 rounded-2xl p-4 space-y-4 shadow-lg">
                                ...
                                <div className="space-y-2">
                                    {tpl.activities.map((act, index) => (
                                        <div key={act.id} className="flex items-center justify-between p-2.5 bg-zinc-950 rounded-xl border border-zinc-800/80 transition-all">
                                            <div className="flex items-center space-x-2.5 pr-2">
                                                <span className="text-xs font-mono text-amber-500 font-bold w-5">{index + 1}.</span>
                                                <span className="text-sm font-medium text-zinc-200">
                                                    {act.name} {act.reps && <span className="ml-1 text-xs px-2 py-0.5 rounded bg-amber-500/10 text-amber-400 font-mono border border-amber-500/20">{cleanRepsText(act.reps)}</span>}
                                                </span>
                                            </div>
                                            
                                            <button 
                                                onClick={() => handleDeleteActivity(tpl.id, act.id)}
                                                className="px-3 py-1.5 bg-red-950/60 hover:bg-red-900/80 text-red-300 rounded-lg text-xs font-semibold border border-red-800/60 flex items-center gap-1 transition-all active:scale-95 shrink-0"
                                            >
